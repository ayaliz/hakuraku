...
import { calculateTargetSpeed, getDistanceCategory, calculateReferenceHpConsumption } from "../utils/speedCalculations";
import { getActiveSpeedModifier, getSkillBaseTime } from "../utils/SkillDataUtils";
    ...
    return useMemo(() => {
        ...
        for (let f = 0; f < frames.length - 1; f++) {
            ...
            for (let i = 0; i < numHorses; i++) {
                ...
                // Downhill Event (Separate from Pace Up/Down/Speed Up/Overtake)
                // We'll treat Downhill Mode as its own event stream logic if we wanted to persistence-check it too, 
                // but the original code just checked it frame-by-frame for the label.
                // The task request specifically mentions "Speed up/Overtake logic... label should stay the same".
                // We will implement the Pace/Speed/Overtake persistence here.
                // If "Downhill Mode" is also a heuristic label, we should probably persist it too. 
                // However, "Downhill Mode" is usually a distinct state (mode 2) in the game logic, inferred here by consumption.
                // Let's focus on Pace/Speed/Overtake for now as they are mutually exclusive in the display logic (or at least handled similarly).
                // Actually, the original code pushes separate items for Downhill and Pace Up. 
                // Let's generate Downhill events too.
                ...
                // --- Mode Logic ---
                // We determine the "Target Mode" for this frame.
                // If we are already in a mode, we check exit conditions.
                // Actually, the request says: "label should stay the same until it disappears".
                // This implies: If I am in "Overtake", I stay in "Overtake" as long as the heuristic condition (speed > target) is met, 
                // even if I become 1st place.
                ...
                if (currentMode) {
                    ...
                    // Exit if condition is no longer met
                    // (Simplification: using the same trigger condition data to check consistency)
                    ...
                    if (isTriggeredHigh) {
                        ...
                        // Front runners probably shouldn't have Pace Down? Original logic applied Pace Down to everyone EXCEPT Front Runners (strategy !== 1 && !isOonige).
                        // Wait, original logic:
                        // if (strategy !== 1 && !isOonige) { ... Pace Up / Pace Down ... } else { ... Speed Up / Overtake ... }
                        // So Front Runners do NOT get Pace Down in the original logic.
                        if (!isFrontRunner) {
        ...
        // Close any open modes at end of loop (or end of race)
